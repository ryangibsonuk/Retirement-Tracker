...
class RT_Shortcodes {
	...
	public static function get_default_inputs() {
		return array(
			...
			'dcPension'         => 180000,
			'sipp'              => 132000,
			'pensionPot'         => 312000, // dcPension + sipp
			'dbPensionAnnual'   => 0,
			'dbPensionAge'      => 65,
			'statePensionAge'    => 67,
	...
	public static function form( $atts ) {
		...
		if ( isset( $_POST['rt_save_scenario'] ) && wp_verify_nonce( $_POST['rt_nonce'] ?? '', 'rt_save_scenario' ) ) {
			$dc  = self::parse_money( $_POST['rt_dc'] ?? '' );
			$sipp = self::parse_money( $_POST['rt_sipp'] ?? '' );
			$raw = array(
				'currentAge'        => (int) ( $_POST['rt_current_age'] ?? 44 ),
				'retirementAge'     => (int) ( $_POST['rt_retirement_age'] ?? 58 ),
				...
				'sipp'              => $sipp,
				'pensionPot'        => $dc + $sipp,
				'dbPensionAnnual'   => self::parse_money( $_POST['rt_db_annual'] ?? '' ),
				'dbPensionAge'      => (int) ( $_POST['rt_db_age'] ?? 65 ),
				'statePensionAge'   => (int) ( $_POST['rt_state_age'] ?? 67 ),
				'statePensionAnnual' => self::parse_money( $_POST['rt_state_annual'] ?? '' ),
				'btlAnnual'         => self::parse_money( $_POST['rt_btl'] ?? '' ),
